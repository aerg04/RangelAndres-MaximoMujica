...
//  
    ...
    // Función para eliminar un nodo de un BST (sin probar)
...
//            deleteNodo(proot.getpR(), key);
    ...
    public Nodo DeleteNode(int key, Nodo ABB){
        Nodo resp=ABB;
        if(key<ABB.getId()){
            ABB.setpL(DeleteNode(key, ABB.getpL()));
        }else{
            if(key>ABB.getId()){
                ABB.setpR(DeleteNode(key, ABB.getpR()));
            }else{
                if(ABB.getpL()!=null && ABB.getpR()!=null){
                    Nodo temp=ABB;
                    Nodo Max_Izq = Max(ABB.getpL());
                    ABB.setId(Max_Izq.getId());
                    temp.setpL(DeleteNode(Max_Izq.getId(), temp.getpL()));
                }else{
                    if(ABB.getpL()!=null){
                        resp=ABB.getpL();
                    }else{
